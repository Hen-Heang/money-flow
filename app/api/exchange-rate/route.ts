...
export async function GET() {
  ...
  try {
    ...
    // Upsert so the table doesn't grow unbounded — exchange_rates grants writes to
    // service_role only (see 20260514_explicit_grants.sql), so the session-scoped
    // client above can only read it; this needs the admin client.
