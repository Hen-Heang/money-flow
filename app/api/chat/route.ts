import { streamText } from 'ai'
import { createServerSupabaseClient } from '@/lib/supabase-server'
import { getChatModel, resolveProvider, type AIProvider } from '@/lib/ai-provider'
import { FALLBACK_EXCHANGE_RATE } from '@/shared/presets'

// ── Rate limiter ─────────────────────────────────────────────────────────────
// In-memory sliding-window: 20 requests per user per minute
const RATE_LIMIT = 20
const WINDOW_MS = 60_000
const MAX_MESSAGES = 50
const MAX_MESSAGE_LENGTH = 2_000

const rateLimitMap = new Map<string, { count: number; resetAt: number }>()

function checkRateLimit(userId: string): { allowed: boolean; retryAfter: number } {
  const now = Date.now()
  const entry = rateLimitMap.get(userId)

  if (!entry || now >= entry.resetAt) {
    rateLimitMap.set(userId, { count: 1, resetAt: now + WINDOW_MS })
    return { allowed: true, retryAfter: 0 }
  }

  if (entry.count >= RATE_LIMIT) {
    return { allowed: false, retryAfter: Math.ceil((entry.resetAt - now) / 1000) }
  }

  entry.count++
  return { allowed: true, retryAfter: 0 }
}

// Purge stale entries every 5 minutes to avoid unbounded memory growth
setInterval(() => {
  const now = Date.now()
  for (const [id, entry] of rateLimitMap) {
    if (now >= entry.resetAt) rateLimitMap.delete(id)
  }
}, 5 * 60_000)
// ─────────────────────────────────────────────────────────────────────────────

export async function POST(req: Request) {
  const supabase = await createServerSupabaseClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()

  if (!user) return new Response('Unauthorized', { status: 401 })

  // Rate limit check
  const { allowed, retryAfter } = checkRateLimit(user.id)
  if (!allowed) {
    return new Response(`Too many requests. Please wait ${retryAfter}s before trying again.`, {
      status: 429,
      headers: { 'Retry-After': String(retryAfter) },
    })
  }

  const body = await req.json()
  const { messages } = body

  // Validate messages array
  if (!Array.isArray(messages) || messages.length === 0) {
    return new Response('messages must be a non-empty array', { status: 400 })
  }
  if (messages.length > MAX_MESSAGES) {
    return new Response(`Conversation too long. Maximum ${MAX_MESSAGES} messages allowed.`, { status: 400 })
  }
  for (const msg of messages) {
    if (!msg || typeof msg !== 'object') return new Response('Invalid message format', { status: 400 })
    if (msg.role !== 'user' && msg.role !== 'assistant') return new Response('Invalid message role', { status: 400 })
    if (typeof msg.content !== 'string') return new Response('Message content must be a string', { status: 400 })
    if (msg.content.length > MAX_MESSAGE_LENGTH) {
      return new Response(`Message too long. Maximum ${MAX_MESSAGE_LENGTH} characters per message.`, { status: 400 })
    }
  }

  // Fetch last 3 months of transactions for context
  const now = new Date()
  const threeMonthsAgo = new Date(now)
  threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3)
  const fromDate = threeMonthsAgo.toISOString().split('T')[0]
  const currentMonthStart = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-01`

  const [{ data: transactions }, { data: exchangeRateRow }] = await Promise.all([
    supabase
      .from('transactions')
      .select('type, amount_krw, amount_usd, date, description, categories(name)')
      .eq('user_id', user.id)
      .gte('date', fromDate)
      .order('date', { ascending: false }),
    supabase
      .from('exchange_rates')
      .select('rate')
      .eq('base_currency', 'USD')
      .eq('target_currency', 'KRW')
      .order('fetched_at', { ascending: false })
      .limit(1)
      .single(),
  ])

  const txList = transactions || []
  const exchangeRate = Number(exchangeRateRow?.rate) || FALLBACK_EXCHANGE_RATE

  // Current month transactions
  const currentMonthTx = txList.filter((t) => t.date >= currentMonthStart)
  const monthIncome = currentMonthTx
    .filter((t) => t.type === 'income')
    .reduce((s, t) => s + Number(t.amount_krw), 0)
  const monthExpense = currentMonthTx
    .filter((t) => t.type === 'expense')
    .reduce((s, t) => s + Number(t.amount_krw), 0)

  // 3-month totals
  const totalIncome = txList
    .filter((t) => t.type === 'income')
    .reduce((s, t) => s + Number(t.amount_krw), 0)

  const totalExpense = txList
    .filter((t) => t.type === 'expense')
    .reduce((s, t) => s + Number(t.amount_krw), 0)

  const catMap: Record<string, number> = {}
  currentMonthTx
    .filter((t) => t.type === 'expense')
    .forEach((t) => {
      const catRaw = t.categories
      const cat = catRaw && !Array.isArray(catRaw) ? (catRaw as { name: string }) : null
      const name = cat?.name || 'Uncategorized'
      catMap[name] = (catMap[name] || 0) + Number(t.amount_krw)
    })

  const topCategories = Object.entries(catMap)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([name, amt]) => `${name}: ₩${amt.toLocaleString()}`)
    .join(', ')

  // Recent 5 transactions for context
  const recentTx = txList
    .slice(0, 5)
    .map((t) => {
      const catRaw = t.categories
      const cat = catRaw && !Array.isArray(catRaw) ? (catRaw as { name: string }) : null
      return `${t.date} | ${t.type} | ₩${Number(t.amount_krw).toLocaleString()} | ${cat?.name || 'Uncategorized'} | ${t.description || ''}`
    })
    .join('\n')

  const today = new Date().toLocaleDateString('en-US', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  })

  const currentMonthLabel = now.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })

  const financialContext =
    txList.length > 0
      ? `
User Financial Summary:

This Month (${currentMonthLabel}):
- Income: ₩${monthIncome.toLocaleString()} (~$${(monthIncome / exchangeRate).toFixed(0)})
- Expenses: ₩${monthExpense.toLocaleString()} (~$${(monthExpense / exchangeRate).toFixed(0)})
- Net: ₩${(monthIncome - monthExpense).toLocaleString()}
- Top Spending Categories: ${topCategories || 'None'}
- Transactions this month: ${currentMonthTx.length}

Last 3 Months (${threeMonthsAgo.toLocaleDateString('en-US', { month: 'short' })} – ${currentMonthLabel}):
- Total Income: ₩${totalIncome.toLocaleString()} (~$${(totalIncome / exchangeRate).toFixed(0)})
- Total Expenses: ₩${totalExpense.toLocaleString()} (~$${(totalExpense / exchangeRate).toFixed(0)})
- Net Balance: ₩${(totalIncome - totalExpense).toLocaleString()}
- Savings Rate: ${totalIncome > 0 ? ((1 - totalExpense / totalIncome) * 100).toFixed(1) : 0}%
- Total Transactions: ${txList.length}
- Exchange rate used: ₩${exchangeRate.toLocaleString()} per $1

Recent Transactions:
${recentTx}
`
      : 'No transaction data available yet.'

  const { data: userPref } = await supabase.from('users').select('ai_provider').eq('id', user.id).single()
  const provider = resolveProvider((userPref?.ai_provider as AIProvider) || 'gemini')

  const result = streamText({
    model: getChatModel(provider),
    system: `You are a friendly and insightful personal finance assistant for the Money Flow app.
Help users understand their spending habits, track budgets, and make smarter financial decisions.
Be concise, practical, and encouraging. Use bullet points for lists. Use ₩ for Korean Won and $ for USD.
If the user asks something unrelated to finance, gently redirect them.
Today's date: ${today}

${financialContext}`,
    messages,
  })

  return result.toTextStreamResponse()
}
